...
const Index = () => {
  ...
  const onKeydownEnter = () => {
    ...
    if (VALID_WORD_LIST.includes(word.toLowerCase())) {
      ...
      const copyWordArray = word.toLowerCase().split('');
      ...
      copyWordArray.forEach((alphabet, index) => {
      ...
      copyWordArray.map((alphabet, index) => {
  ...
  return (
    <Wrapper>
      ...
      <Keyboard
        ...
        words={words}
        keyState={rowState[rowState.length - 1] as BoxState[]}
      />
      {isShowAlert && (
        <AlertPortal>
